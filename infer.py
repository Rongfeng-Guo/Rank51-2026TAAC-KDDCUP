...
_FALLBACK_MODEL_CFG = {
    ...
    # HashGate / hashed sequence embeddings. Must match train.py defaults.
    'seq_hash_bucket_size': 0,
    'seq_hash_gate_init': -1.0,
    'seq_hash_allowlist': '',
    'ns_tokenizer_type': 'rankmixer',
    'user_ns_tokens': 0,
    'item_ns_tokens': 0,
    # NOTE: USER_DENSE_SMOOTH_CLIP - this adds user_dense_smooth_clip
    # scale/bias parameters when active; old checkpoints fall back to False.
    'use_user_dense_smooth_clip': False,
    # NOTE: USER_DENSE_GROUP_PROJECTOR - Older checkpoints predate the grouped
    # dense-token replacement, so fallback keeps the original single token.
    'use_user_dense_group_projector': False,
    'dense_stat_log_clamp_max': 20.0,
    'dense_quantile_mid_dim': 64,
    'dense_group_dropout': 0.05,
}
...
def append_periodic_time_ns_to_user_groups(
    ...
    # NOTE: PERIODIC_TIME_NS - inference mirrors train.py by merging the
    # timestamp-derived synthetic categorical features into the last existing
    # user group, preserving the trained User NS token count.
    time_indices = []
    for fid in PERIODIC_TIME_NS_FIDS:
        if fid not in user_fid_to_idx:
            raise KeyError(
                f"Periodic time NS fid={fid} is missing from user_int_schema; "
                "check train_config.json/use_periodic_time_ns consistency")
...
def append_item_id_feature_to_item_groups(
    ...
    # NOTE: ITEM_ID_FEATURE - inference mirrors train.py by merging the hashed
    # raw-item_id synthetic feature into the last existing item group,
    # preserving the trained Item NS token count.
    if ITEM_ID_FEATURE_FID not in item_fid_to_idx:
        raise KeyError(
            f"Synthetic item_id fid={ITEM_ID_FEATURE_FID} is missing from item_int_schema; "
            "check train_config.json/use_item_id_feature consistency")
...
def build_model(
    ...
    Args:
        ...
        use_periodic_time_ns: whether the dataset appended timestamp-derived
            synthetic categorical user_int features.
        use_item_id_feature: whether the dataset appended a hashed synthetic
            raw-item_id categorical item_int feature.
        device: torch device.
    """
    ...
    if use_periodic_time_ns:
        # NOTE: PERIODIC_TIME_NS - keep inference user grouping identical to
        # training so strict checkpoint loading sees the same tokenizer shapes.
        user_ns_groups = append_periodic_time_ns_to_user_groups(
            user_ns_groups, user_fid_to_idx)
    if use_item_id_feature:
    ...
    model = PCVRHyFormer(
        ...
        # Needed by HashGate allowlist: train.py passes pcvr_dataset.sideinfo_fids.
        seq_feature_ids=dataset.sideinfo_fids,
        user_ns_groups=user_ns_groups,
        item_ns_groups=item_ns_groups,
        # NOTE: USER_DENSE_GROUP_PROJECTOR - grouped dense projection locates
        # fid spans from the same schema metadata train.py used.
...
def main() -> None:
    ...
    # NOTE: PERIODIC_TIME_NS - this switch changes dataset-exposed user_int
    # dimensions, so evaluation must mirror the training-side setting.
    use_periodic_time_ns = bool(train_config.get('use_periodic_time_ns', False))
    use_seq_periodic_hour_day_sideinfo = bool(
        train_config.get('use_seq_periodic_hour_day_sideinfo', False))
    # NOTE: ITEM_ID_FEATURE - this switch changes dataset-exposed item_int
    # dimensions and tokenizer shapes, so infer.py must mirror train_config.
    use_item_id_feature = bool(train_config.get('use_item_id_feature', False))
    item_id_hash_buckets = int(train_config.get(
        'item_id_hash_buckets', ITEM_ID_HASH_BUCKETS_DEFAULT))
